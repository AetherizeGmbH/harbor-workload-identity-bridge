...
- Managed clusters (GKE, EKS AL2023, AKS) install with the default
  ...
  cloud's own provider (verified in e2e by asserting cloud-registry
  pulls still work after install — see ADR-0022).
...
- [ADR-0022](0022-gke-e2e-harness.md) — the e2e harness that verifies
  merge mode on a real managed cluster.
