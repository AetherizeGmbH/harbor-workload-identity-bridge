...
3. Platform support matrix (documented in `docs/platforms.md`):
   ...
   | GKE Standard (COS / Ubuntu) | `auto` → merge into `cri_auth_config.yaml` | unit-tested merge, not run |
...
- The support claims match what the code can do. Unverified platforms are
  marked as such. A managed-cloud e2e harness should reuse the shared e2e
  modules (`test/e2e/modules/*`) with a cloud-specific cluster module; GKE is
  the first one to verify.
